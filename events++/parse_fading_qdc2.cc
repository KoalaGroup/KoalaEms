/*
 * ems/events++/parse_fading_qdc2.cc
 * 
 * created 2006-Oct-25 PW
 */
...
VERSION("2006-Oct-25", __FILE__, __DATE__, __TIME__,
"$ZEL$")
#define XVERSION
...
{
//cout<<"spectrum("<<name<<")::add int "<<val<<endl;
    sum+=(double)val;
...
{
//cout<<"spectrum("<<name<<")::add double "<<val<<endl;
    sum+=val;
    qsum+=val*val;
    num++;
}
...
parse_val(ems_u32 v, unsigned int id)
{
    ...
    if (ident==0) { // raw
        ...
        case 0x37: { // fixed window
                ...
                else if (x37[id][mod][ch]==1)
                    b->cs[id][mod][ch].base1.add(val);
                else if (x37[id][mod][ch]==2)
                    b->cs[id][mod][ch].base3.add(val);
                x37[id][mod][ch]++;
            }
            break;
        default: {
...
//---------------------------------------------------------------------------//
//die LP Events kannst Du mittels eines Tests auf Crate2 (IS 8), Slot6, Channel
//15 Integral > 1000 aussortieren.
static bool
subevent_good(const ems_subevent* sev)
{
    unsigned int* d=sev->data+4;
...
//---------------------------------------------------------------------------//
static bool
event_good(const ems_event* event)
{
    ems_subevent* sev=event->subevents;
    while (sev) {
        if (sev->sev_id==8) {
            return subevent_good(sev);
        }
        sev=sev->next;
    }
    return false;    
...
raus:
    ...
    cerr<<good_events<<" good events"<<endl;

    ofstream ff("mistx/log");
    if (!ff.good()) {
        cerr<<"nix file \""<<"mistx/log"<<"\""<<endl;
    }
    ...
    for (int id=0; id<10; id++) {
        for (int mod=0; mod<16; mod++) {
            for (int ch=0; ch<16; ch++) {
                ...
                double N[4]={0,0,0,0};
                b=bb[0];
                if (b->cs[id][mod][ch].base.num<1)
                    continue;
                char s[100];
                snprintf(s, 100, "mistx/%d-%02d-%02d", id, mod, ch);
                ofstream f(s);
                if (!f.good()) {
                ...
                for (unsigned int i=0; i<ns; i++) {
                    ...
                    b->cs[id][mod][ch].base1.calculate();
                    b->cs[id][mod][ch].base3.calculate();
                    ...
                    f<<i
                        ...
                        <<"\t"<<b->cs[id][mod][ch].base1.average//9
                        <<"\t"<<b->cs[id][mod][ch].base3.average//9
                        <<endl;
                    int idx=(i*4)/ns;
