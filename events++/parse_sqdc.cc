...
"$ZEL: parse_sqdc.cc,v 1.2 2006/06/21 12:25:25 wuestner Exp $")
#define XVERSION
...
parse_sqdc(unsigned int v, int mod, int chan)
{
    int code=(v>>16)&0x3f;
    if (code==0) {
        pulse_[chan][sqdc_idx[chan]]=v&0xfff;
        if (sqdc_idx[chan]==0) N[chan]++;
        if (sqdc_idx[chan]==7) x7++;
        if (sqdc_idx[chan]==13) x13++;
        if (sqdc_idx[chan]==23) x23++;
        sqdc_idx[chan]++;
...
raus:
    ...
    cerr<<"x7="<<x7<<" x13="<<x13<<" x23="<<x23<<endl;
