/*
 * ems/events++/tsm_cl2tade_nosplit.cc
 * 
...
"$ZEL: tsm_cl2tade_nosplit.cc,v 1.1 2005/02/11 15:45:17 wuestner Exp $")
#define XVERSION
...
try_extract(ems_object& ems, int final, compressed_output* of, int* nevents)
{
    while (1) {
        ...
        int res=cl2tade::decode_to_tade_nosplit(event, ems,
            of[0].file(), of[1].file(), of[2].file(), of[3].file(), of[4].file(),
            nevents);
        if (res<0)
            return res;
        ems.next_evno++;
...
{
    dsUint32_t sh;
    compressed_output output[5];
    const char *stub[]={"all", "even", "odd", "scaler", "d15d16"};
    int nevents[5]={0, 0, 0, 0, 0};
    ems_object ems;
    ems_u32 *buf;
    int path;
    int res, i, rres, wres;
