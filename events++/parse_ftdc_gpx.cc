...
"$ZEL: parse_ftdc_gpx.cc,v 1.1 2006/10/19 13:12:43 wuestner Exp $")
#define XVERSION
...
subevent_end(void)
{
    for (int chan=0; chan<1024; chan++) {
        if (nhit[chan]==2) {
            int diff=(hits[chan][1]&0xffff)-(hits[chan][0]&0xffff);
            if (diff>0) {
                hist[chan][diff]++;
                N[chan]++;
            }
