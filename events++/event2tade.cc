...
"$ZEL: event2tade.cc,v 1.1 2005/02/11 15:45:04 wuestner Exp $")
#define XVERSION
...
int
decode_to_tade(deque<sev_object>& evdata, ems_object& ems,
        FILE* tade_even, FILE* tade_odd, FILE* tade_caler,
        FILE* tade_d15d16, int* nevents)
{
    ...
    if (true/*event.trigger&8*/) { // trigger event
        int n=event.to_scalerfile(tade_caler);
