...
do_dump(input_event_buffer_struct *ibs)
{
    ...
    if (data32[0]<9) {
        fprintf(stderr, "event %d too short (%d words)\n", data32[1], data32[0]);
        return;
    }
    if ((data32[0]!=data32[5]+5)||
        (data32[0]!=data32[6]+6)||
        ((data32[7]&0xffff)!=data32[6]*4)) {
        fprintf(stderr, "inconsistent data in event %d\n", data32[1]);
    }
    if (data32[6]-3!=324) {
        fprintf(stderr, "wrong length %d of event %d\n", data32[6]-3, data32[1]);
        return;
    }

#if 0
    for (i=10, j=0; i<size; i++) {
        uint32_t d=data32[i];
        if (!(d&0x08000000) && ((d&0x07ff0000)!=0x7ff0000))
