/*
 * ems/events++/qdc_powerspect.cc
 * 
 * created 2006-May-30 PW
 */
...
VERSION("2006-May-30", __FILE__, __DATE__, __TIME__,
"$ZEL: qdc_powerspect.cc,v 1.1 2006/06/21 12:24:45 wuestner Exp $")
#define XVERSION
...
subevent_end(void)
{
    for (int chan=0; chan<16; chan++) {
        if (qdc_idx[chan]==0)
            continue;
        if (qdc_idx[chan]!=nn) {
            cerr<<"qdc_idx["<<chan<<"]="<<qdc_idx[chan]<<endl;
            return -1;
        } else {
            double sum=0.;
            for (int j=0; j<nn; j++) {
                fftData.in[j]=pulse_[chan][j];
                sum+=fftData.in[j];
            }
            sum/=nn;
            for (int j=0; j<nn; j++) {
            ...
            int HalfSize=fftData.getHalfSize();
            for (int j=0; j<HalfSize; j++){
                pulse[chan][j]+=fftData.power_spectrum[j];
            }
            N[chan]++;
        }
    }
