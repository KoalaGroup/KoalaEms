/*
 * ems/events++/tsm_cl2tade_2007.cc
 * 
...
#include "event2tade_2007.hxx"
#include <versions.hxx>

VERSION("2007-Jun-26", __FILE__, __DATE__, __TIME__,
"$ZEL: tsm_cl2tade_2007.cc,v 1.2 2007/06/27 00:17:36 wuestner Exp $")
#define XVERSION
...
//---------------------------------------------------------------------------//
int
try_extract(ems_object& ems, int final, int nr_files,
    compressed_output* of, int* nevents)
{
    while (1) {
        if (!final) {
        ...
        int res=cl2tade::decode_to_tade_2007(event, ems, nr_files,
            of, nevents);
        if (res<0)
            return res;
        ems.next_evno++;
...
//---------------------------------------------------------------------------//
int
parse_events(ems_u32* buf, int num, ems_object& ems, int nr_files,
    compressed_output* of, int* nevents)
{
    ...
    if (empty) {
        int res;
        res=try_extract(ems, 0, nr_files, of, nevents);
        if (res<0)
            return -1;
    }
...
//---------------------------------------------------------------------------//
int
parse_no_more_data(ems_u32* buf, int num, ems_object& ems, int nr_files,
    compressed_output* of, int* nevents)
{
    printf("no_more_data\n");
    try_extract(ems, 1, nr_files, of, nevents);
    return 0;
}
//---------------------------------------------------------------------------//
int
parse_record(ems_u32* buf, int num, ems_object& ems, int nr_files,
    compressed_output* of, int* nevents)
{
    int res=0;
    switch (buf[0]) {
    case clusterty_events:
        res=parse_events(buf+1, num-1, ems, nr_files, of, nevents);
        break;
    ...
    case clusterty_no_more_data:
        res=parse_no_more_data(buf+1, num-1, ems, nr_files, of, nevents);
        break;
    default:
...
archive_file(dsUint32_t sh, int p, struct l_file *ff, const char *stub, int events)
{
    ...
    if (archive_file(sh, p, &stbuf, &obj_name, emsinfo.digest, dsmFalse,
            0, 0, 0, 0)<0) {
...
process_file(struct l_file *ff)
{
    dsUint32_t sh;
    int nr_files;
    compressed_output *output=0;
    const char *stub[]={"evt", "scaler"};
    int *nevents=0;
    ems_object ems;
    ems_u32 *buf;
    int path;
    ...
    nr_files=sizeof(stub)/sizeof(char*);
    output=new compressed_output[nr_files];
    nevents=new int[nr_files];
    ...
    ostringstream st;
    st<<tmpdir<<"/run_"<<setw(4)<<setfill('0')<<ff->info.x.ems.runnr;
    ...
    if (res<0) {
        close(path);
        delete[] output;delete[] nevents;return -1;
    }
    ...
    do {
        ...
        if (rres>0) {
            ...
            wres=parse_record(buf+2, rres-2, ems, nr_files, output, nevents);
            delete[] buf;
            if (wres) {
                error=1;
                break;
