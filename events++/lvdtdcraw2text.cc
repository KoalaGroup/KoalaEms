/*
 * ems/events++/lvdtdcraw2text.cc
 * 
 * created 2006-Jun-23 PW
 */
...
{
    for (int i=0; i<1024; i++) {
        long long int T0, T1, T2, Th;
        long long int t0, t1, t2, th;
        long long int dth, dt0, dt, dl, last;
        double dd;
        ...
        cout<<i<<" "<<hits<<endl
            <<(cdata[i].hits[0].edge?"  R":"  F")
            <<" "<<setw(9)<<cdata[i].hits[0].t2
            <<" "<<setw(2)<<cdata[i].hits[0].t1
            <<" "<<setw(5)<<cdata[i].hits[0].t0<<endl;
        T0=cdata[i].hits[0].t0;
        T1=cdata[i].hits[0].t1;
        T2=cdata[i].hits[0].t2;
        Th=T1+(T2<<6);
        last=0;
        for (int j=1; j<cdata[i].num_hits; j++) {
            t0=cdata[i].hits[j].t0;
            t1=cdata[i].hits[j].t1;
            t2=cdata[i].hits[j].t2;
            th=t1+(t2<<6);
            dth=th-Th;
            dt0=t0-T0;
            dt=dth*0xc060+dt0;
            ...
            cout<<(cdata[i].hits[j].edge?"  R":"  F")
                <<" "<<setw(9)<<cdata[i].hits[j].t2
                <<" "<<setw(2)<<cdata[i].hits[j].t1
                <<" "<<setw(5)<<cdata[i].hits[j].t0
                <<"  "<<setw(9)<<dt
                <<"  "<<setw(9)<<dl
                <<"  "<<dd<<" ns"<<endl;
            last=dt;
...
{
    for (unsigned int i=4; i<sev->size; i+=2) {
        ems_u32 v=sev->data[i];
        ems_u32 v1=sev->data[i+1];
        int mod_chan, edge;
        ems_u32 t0, t1, t2;
        ...
        if (n<MAXHITS) {
            cdata[mod_chan].hits[n].t0=t0;
            cdata[mod_chan].hits[n].t1=t1;
            cdata[mod_chan].hits[n].t2=t2;
...
parse_event(const ems_event* event)
{
    ...
    while (sev) {
        if (parse_tdc(sev)<0)
            return -1;
        cerr<<"event "<<event->event_nr<<endl;
        sev=sev->next;
    }
