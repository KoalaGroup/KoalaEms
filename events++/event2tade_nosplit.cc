/*
 * ems/events++/event2tade_nosplit.cc
 * 
 * created 2004-12-12 PW
 */
...
VERSION("Dec 12 2004", __FILE__, __DATE__, __TIME__,
"$ZEL: event2tade_nosplit.cc,v 1.1 2005/02/11 15:45:05 wuestner Exp $")
#define XVERSION
...
{
    fprintf(f, "%d %d %d %d  Event\n",
            evno, tqmap.size(), runno, triggerPattern);
...
int
Event::to_d15d16(FILE* f)
{
    int n=0;
    map<det_elem, tqval>::iterator iter=event.tqmap.begin();
    while (iter!=event.tqmap.end()) {
        const det_elem& elem=(*iter).first;
        if ((elem.first==15) || (elem.first==16))
            n++;
        iter++;
    }
    if (!n)
        return 0;
    fprintf(f, "%d %d %d %d  Event\n",
            evno, n, runno, triggerPattern);
    iter=event.tqmap.begin();
    while (iter!=event.tqmap.end()) {
        tqval& tqv=(*iter).second;
        const det_elem& elem=(*iter).first;
        if ((elem.first==15) || (elem.first==16))
...
int
decode_to_tade_nosplit(deque<sev_object>& evdata, ems_object& ems,
        FILE* tade_all,
        FILE* tade_even, FILE* tade_odd, FILE* tade_scaler,
        FILE* tade_d15d16, int* nevents)
{
    static int count=0;
    if (evdata.empty()) {
        if (ems.next_evno>0)
    ...
    if (event.trigger&1) { // regular event
        {
            int n=event.to_tadefile(tade_all);
            if (nevents) nevents[0]+=n;
        }
        if (event.evno&1) {
            int n=event.to_tadefile(tade_odd);
            if (nevents) nevents[1]+=n;
        } else {
            int n=event.to_tadefile(tade_even);
            if (nevents) nevents[2]+=n;
        }
        int n=event.to_d15d16(tade_d15d16);
            if (nevents) nevents[4]+=n;
    }
    if (true/*event.trigger&8*/) { // trigger event
        int n=event.to_scalerfile(tade_scaler);
