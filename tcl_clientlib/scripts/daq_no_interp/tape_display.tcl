# $Id: tape_display.tcl,v 1.14 2000/08/10 09:26:49 wuestner Exp $
# copyright: 1998, 2000
...
proc add_DLT_to_display {f key} {
  ...
  entry $f.s.r_e -relief sunken -state disabled -justify right\
      -textvariable global_tapestat_var_rewrites($key)
#    label $f.s.e_l -text {Corrected errors}
#    entry $f.s.e_e -relief sunken -state disabled -justify right\
...
proc add_UNKNOWN_to_display {f key} {
  global global_tapestat_frame_pos global_tapestat_frame_color
...
proc add_tape_to_display {key} {
  ...
  switch [set global_dataout_${key}(type)] {
    DLT4000 -
    DLT7000 {add_DLT_to_display $f $key}
    EXABYTE {add_EXA_to_display $f $key}
    default {
      output "tape [set global_dataout_${key}(tape)] unknown"
      add_UNKNOWN_to_display $f $key
    }
  }
...
proc add_do_to_tapedisplay {ved idx} {
  ...
  if {[lindex $dom 3]=="tape"} {
    set type [$ved command1 TapeInquiry $idx | stringlist]
