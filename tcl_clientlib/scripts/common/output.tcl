...
proc init_output {title} {
  global global_output
  if [winfo exists .output] return
...
proc create_output {} {
  ...
  wm withdraw $w
  wm title $w $global_output(title)
  wm iconname $w $global_output(title)
  wm geometry $w 400x300
  wm maxsize $w 10000 10000
...
proc output {text} {
  ...
  if {![winfo exists $global_output(win)]} create_output
