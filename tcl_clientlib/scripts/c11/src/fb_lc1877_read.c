...
*  3. 1.95  max. number of words reduced (1537 --> 769) to 8 hits/channel      *
*  3. 1.95  multiblock readout routines added                                  *
* 18. 1.95  "end of conversion"-test modified                                  *
*  1. 2.95  Unsol_Data replaced by Unsol_Warning                               *
* 21. 3.95  multiblock included in "fb_lc1877_readout"                         *
*           new standard readout version: "fb_lc1877_readout_check"            *
* 29. 9.95  max. number of words increased (769 --> 1537) to 16 hits/channel   *
* 24. 2.97  "fb_lc1877_readout_pl" added                                       *
*           new standard readout version: "fb_lc1877_readout_pl"               *
*                                                                              *
*******************************************************************************/
...
plerrcode proc_fb_lc1877_check_buf(p)
...
return(lc1877_buf(pat,1,eventcnt));
}

/******************************************************************************/

plerrcode test_proc_fb_lc1877_check_buf(p)
...
plerrcode proc_fb_lc1877_read_ptr(p)
...
plerrcode test_proc_fb_lc1877_read_ptr(p)
...
plerrcode proc_fb_lc1877_readout_t2(p)
...
if (wt_con==-1) {                   /* time elapsed: conversion not completed */
  ...
  { int unsol_mes[5],unsol_num;
    ...
    unsol_mes[2]=eventcnt;unsol_mes[3]=LC_TDC_1877;unsol_mes[4]=con_pat;
...
plerrcode test_proc_fb_lc1877_readout_t2(p)
...
while (pat) {                           /* check patt: IS module definition   */
  if (pat&0x1) {
    ...
    do {
      if (pa==memberlist[j]) {
        if (get_mod_type(memberlist[j])!=LC_TDC_1877) {
          *outptr++=0;*outptr++=pa;
          return(plErr_BadModTyp);
         }
        isml_def++;
...
plerrcode proc_fb_lc1877_readout_t1(p)
  /* readout of fastbus lecroy tdc1877 using module pattern & list */
int *p;
{
#define MAX_WT_CON  100                 /* wait for conversion: max. timeout  */
...
plerrcode test_proc_fb_lc1877_readout_t1(p)
...
plerrcode proc_fb_lc1877_readout_pl(p)
  /* readout of fastbus lecroy tdc1877 using module pattern & list */
int *p;
{
#define MAX_WT_CON  100                 /* wait for conversion: max. timeout  */
...
if (wt_con==-1) {                   /* time elapsed: conversion not completed */
  ...
  if (p[4]) {
    wc= FRDB_S(p[5],outptr,(p[6]*MAX_WC_PL)) - 1;
    if (sscode() != 2) {
      if (sscode() == 0)
       { int unsol_mes[5],unsol_num;
         unsol_num=5;unsol_mes[0]=8;unsol_mes[1]=3;
         unsol_mes[2]=eventcnt;unsol_mes[3]=LC_TDC_1877;unsol_mes[4]=wc;
         send_unsolicited(Unsol_Warning,unsol_mes,unsol_num);
        }
      else {
...
plerrcode test_proc_fb_lc1877_readout_pl(p)
...
plerrcode proc_fb_lc1877_readout_check(p)
...
if (wt_con==-1) {                   /* time elapsed: conversion not completed */
  ...
  DV(D_DUMP,printf("TDC 1877: end of conversion after %d/%d\n",(MAX_WT_CON-wt_con),MAX_WT_CON);)
  if ((buf_check=lc1877_buf(pat,2,eventcnt))!=plOK) {
    outptr--;*outptr++=3;
    return(buf_check);
   }
  ...
  if (p[3]) {
    wc= FRDB_S(p[4],outptr,(p[5]*MAX_WC_PL)) - 1;
    if (sscode() != 2) {
      if (sscode() == 0)
       { int unsol_mes[5],unsol_num;
         unsol_num=5;unsol_mes[0]=8;unsol_mes[1]=3;
...
plerrcode test_proc_fb_lc1877_readout_check(p)
...
char name_proc_fb_lc1877_readout[]="fb_lc1877_readout";
int ver_proc_fb_lc1877_readout=1;

/******************************************************************************/

static procprop fb_lc1877_readout_prop= {1,-1,0,0};

procprop* prop_proc_fb_lc1877_readout()
{
return(&fb_lc1877_readout_prop);
}
...
plerrcode proc_fb_lc1877_readout(p)
  /* readout of fastbus lecroy tdc1877 */
int *p;
{
#define MAX_WT_CON  100                 /* wait for conversion: max. timeout  */
...
plerrcode test_proc_fb_lc1877_readout(p)
  /* test subroutine for "fb_lc1877_readout" */
int *p;
{
int pat;                                /* IS module pattern                  */
int n_mod;                              /* number of modules                  */
