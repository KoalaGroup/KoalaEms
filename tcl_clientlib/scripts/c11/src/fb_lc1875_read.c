...
*                                                                              *
* 10. 5.95                                                                     *
* 24. 8.95  sparsification implemented                                         *
* 24. 2.97  "fb_lc1875_readout_pl" added                                       *
*           new standard readout version: "fb_lc1875_readout_pl"               *
*                                                                              *
*******************************************************************************/
...
plerrcode proc_fb_lc1875_readout_pl(p)
...
if (wt_con==-1) {                   /* time elapsed: conversion not completed */
  ...
  unsol_mes[2]=eventcnt;unsol_mes[3]=LC_TDC_1875;unsol_mes[4]=con_pat;
        ...
        if (sscode()!=2) {              /* test ss code                       */
          if (sscode()==0)
           { int unsol_mes[5],unsol_num;
             ...
             unsol_mes[2]=eventcnt;unsol_mes[3]=LC_TDC_1875;unsol_mes[4]=wc;
             send_unsolicited(Unsol_Warning,unsol_mes,unsol_num);
            }
          else {
...
plerrcode test_proc_fb_lc1875_readout_pl(p)
...
while (pat) {                            /* check: module defined in IS       */
  if (pat&0x1) {
    ...
    do {
      if (pa==memberlist[j]) {
        if (get_mod_type(memberlist[j])!=LC_TDC_1875) {
          *outptr++=0;*outptr++=pa;
...
plerrcode proc_fb_lc1875_readout(p)
...
plerrcode test_proc_fb_lc1875_readout(p)
