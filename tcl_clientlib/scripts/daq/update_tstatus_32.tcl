# $ZEL: update_tstatus_32.tcl,v 1.14 2007/04/15 13:15:09 wuestner Exp $
# copyright 2000
#   Peter Wuestner; Zentrallabor fuer Elektronik; Forschungszentrum Juelich
#

proc update_tstatus_32 {key status} {
  ...
  switch [set global_dataout_${key}(type)] {
    ...
    DLT8000 {
      ...
      if {$len>=20} {
        ...
        set fvar [expr $mbhost*1048576.+$bhost]

        set mbtape [lindex $status 15]
        set btape [lindex $status 16]
        set ftvar [expr $mbtape*1048576.+$btape]

        set global_tapestat_var_tape($key) [xfformat $ftvar]

        if {$global_tapestat_var_time($key)>0} {
          set global_tapestat_var_rate($key) [
              xfformat [
                  expr {int(($fvar-$global_tapestat_var_host_letzt($key))
                    /($jetzt-$global_tapestat_var_time($key)))}]]
          set trate [
                  expr {int(($ftvar-$global_tapestat_var_tape_letzt($key))
                    /($jetzt-$global_tapestat_var_time($key)))}]
        } else {
          set global_tapestat_var_rate($key) ???
          set trate 0
        }
        set global_tapestat_var_host_letzt($key) $fvar
        set global_tapestat_var_tape_letzt($key) $ftvar
        set global_tapestat_var_host($key) [xfformat $fvar]
        ...
        if {$len>=21} {
          ...
          set frest [expr [lindex $status 20]*4096.]
          set global_tapestat_var_rest($key) [xfformat $frest]
          set end_secs 0
          if {$trate>0} {
#            set end_secs [calculate_tapeend $key $ftvar $frest [expr [doubletime]-$t0]]
            set rest_secs [expr $frest/$trate]
            if {$rest_secs>32000000.} {
              set global_tapestat_var_end($key) "after $rest_secs s"
            } else {
              set end_secs [expr [clock seconds] + int($rest_secs)]
