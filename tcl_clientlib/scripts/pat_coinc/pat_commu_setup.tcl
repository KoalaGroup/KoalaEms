# $ZEL: pat_commu_setup.tcl,v 1.1 2002/09/26 12:15:11 wuestner Exp $
...
proc commu_reset {window} {
  ...
  set static_commu(ved)       $global_setup(ved)
  set static_commu(var)       $global_setup(var)
  commu_pack $window
}
...
proc commu_pack {window} {
  ...
  pack forget $window.unixframe $window.tcpframe $window.tcpframe\
      $window.vedframe $window.b_frame
  ...
  pack $window.vedframe -side top -fill x -expand 1
...
proc commu_apply {} {
  ...
  set global_setup(ved)             $static_commu(ved)
  set global_setup(var)             $static_commu(var)
  restart_loop
}
...
proc commu_win_create {} {
  ...
  frame $self.vedframe -relief raised -borderwidth 1
...
# vedframe
  label $self.vedframe.ln -text "VEDname:" -anchor w
  entry $self.vedframe.en -textvariable static_commu(ved) -width 28
  label $self.vedframe.lv -text "Variable:" -anchor w
  entry $self.vedframe.ev -textvariable static_commu(var) -width 28
    grid $self.vedframe.ln $self.vedframe.en -sticky ew
    grid $self.vedframe.lv $self.vedframe.ev -sticky ew
