...
proc channelwin_chan_add {channel row} {
  ...
  global static_channels_name static_channels_vis static_channels_pane
  ...
  grid $w.idx_$channel $w.name_$channel $w.vis_$channel -sticky e -row $row

  for {set i 0} {$i<$global_setup(num_panes)} {incr i} {
    radiobutton $w.pane_($channel)_$i -variable static_channels_pane($channel)\
        -value $i -selectcolor black
    grid $w.pane_($channel)_$i -row $row -column [expr 3+$i]
  }
}
...
proc channelwin_fill_grid {} {
  ...
  grid $w.idx $w.name $w.visible
  for {set i 0} {$i<$global_setup(num_panes)} {incr i} {
    label $w.pane_$i -text "Pane $i" -anchor w
    grid $w.pane_$i -row 0 -column [expr 3+$i]
  }
...
proc channel_win_create {} {
  global global_setup win_pos static_channels
