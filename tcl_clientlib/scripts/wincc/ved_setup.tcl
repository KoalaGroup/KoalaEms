#$Id: ved_setup.tcl,v 1.1 2000/07/15 21:34:40 wuestner Exp $
# © 1999 P. Wüstner; Zentrallabor für Elektronik; Forschungszentrum Jülich
#
# global vars:
#
# global_setup(ved_name)
# win_pos($win)
#

proc dummy_ved_setup {} {}

namespace eval ved_setup {

variable win .xh_ved_setup
variable name

proc reset {} {
  global global_setup
  variable name
  set name $global_setup(ved_name)
}
...
proc ok {} {
  ...
  variable win
  variable name
  set global_setup(ved_name) $name
  wm withdraw $win
}
...
proc win_create {} {
  ...
  wm withdraw $win
  wm group $win .
  wm title $win "WINCC ved"
  wm maxsize $win 10000 10000

  label $win.h -relief raised -borderwidth 1 -text "Name of VED:"
