...
VERSION("2007-04-15", __FILE__, __DATE__, __TIME__,
"$ZEL: eventrate.cc,v 2.20 2010/09/10 22:22:28 wuestner Exp $")
#define XVERSION
...
void (*show_data)(struct timeval&, unsigned int, InvocStatus, float);
...
int readargs()
{
    ...
    args->addoption("interval", "i", 10, "interval in (milli)seconds", "interval");
    args->addoption("milliinterval", "milli", false, "interval is in milliseconds", "milli");
...
/*****************************************************************************/
void sendbytes(const void* vb, int num)
{
    int sent=0, res;
    const char* b=static_cast<const char*>(vb);
    /* if (!b[num]) num--; (what is it for?) */
    do {
...
/*****************************************************************************/
void 
show_data_print(struct timeval& jetzt, unsigned int event, InvocStatus status,
        float rate)
{
    ...
    if (!nomist || (rate>0) || (rate!=lastrate) || (status!=laststate)) {
        ...
        if ((status>=Invoc_error) && (status<=Invoc_active))
            ...
            cout << "unknown state: " << static_cast<int>(status);
        cout << "event " << event;
...
void show_data_send(struct timeval& jetzt, unsigned int event, InvocStatus status,
    float rate)
{
...
timeout.tv_sec=static_cast<time_t>(truncf(interval));
timeout.tv_usec=static_cast<suseconds_t>((interval-timeout.tv_sec)*1000000.);
...
while (running)
  ...
  if (!first) {
    ...
    int res;
    do {
        res=select(0, 0, 0, 0, &to);
    } while ((res<0) && (errno==EINTR) && running);
    if ((res<0) && running) {
...
finterval=args->getintopt("interval");
if (args->getboolopt("milliinterval"))
    finterval/=1000.;
