...
VERSION("Feb 02 2004", __FILE__, __DATE__, __TIME__,
"$ZEL: dump_ved.cc,v 1.3 2004/11/26 22:20:36 wuestner Exp $")
#define XVERSION
...
decode_readoutlist(C_instr_system* is,  C_confirmation* conf)
{
    ...
    int i=3;
    while (i<conf->size()) {
        cout<<"      "<<is->procname(conf->buffer(i++))<<" {";
        int n=conf->buffer(i++);
        for (int j=0; j<n; j++) {
            if (j>0) cout<<' ';
            cout<<conf->buffer(i++);
