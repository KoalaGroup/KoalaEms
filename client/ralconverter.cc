...
VERSION("Sep 05 2000", __FILE__, __DATE__, __TIME__,
"$ZEL: ralconverter.cc,v 1.2 2005/02/11 15:45:09 wuestner Exp $")
#define XVERSION

/*****************************************************************************/

C_readargs* args;
int use_infile, use_outfile;
STRING infile, ofile;
STRING vedhost;
C_VED* ved;
...
use_outfile=!args->isdefault("ofile");
...
for (int i=0; i<num; i++)
  ...
  switch (code)
    ...
    case 0:
      ...
      break;
    case 4: incr(col, row, chan, slice); break;
    case 5:
      if (chan)
        {row=0; col=0; slice++;}
      else
