...
VOID dosallot()
{
sp->CSTR = (CSTR) malloc((unsigned) sp->NUM32);
}
...
{
register char* s;
s=(char*)malloc(strlen(sp->CSTR)+1);
strcpy(s, sp->CSTR);
spush(s, CSTR);
}
...
s = spop(CSTR);
sp->INT32 = (STREQ(sp->CSTR, s) ? TRUE : FALSE);
}
...
s = spop(CSTR);
sp->INT32 = (INT32) strcmp(sp->CSTR, s);
}
