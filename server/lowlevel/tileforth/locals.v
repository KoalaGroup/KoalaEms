...
COMPILATION_CODE(parenunlinksemicolon, parenunlink, "(unlink;)",
    doparenunlinksemicolon)
...
COMPILATION_CODE(parenunlinkdoes, parenunlinksemicolon, "(unlinkdoes>)",
    doparenunlinkdoes)
...
COMPILATION_CODE(parenlocalfetch, parenlocalstore, "(local@)",
    doparenlocalfetch)
...
if (theframed)
  {
  sprintf(s_err, "%s%s: illegal argument binding\n", input_info(),
      theframed -> name);
  emit_err();
  doremovelocals();
  doabort();
    ...
    if (STREQ(tos.CSTR, "}"))
      ...
      if (STREQ(tos.CSTR, "--"))
        ...
        if (argflag)
          ...
          locals++;
        make_entry(tos.CSTR, (INT32) LOCAL, (INT32) COMPILATION,
            arguments + locals);
        }
      }
    }
