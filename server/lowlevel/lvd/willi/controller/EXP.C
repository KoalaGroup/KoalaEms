...
{
	int		ret;
	u_long	cmask[2*C_UNIT]={0};	// anzuzeigende Kanaele
	u_int		i,j,sp;
	int		cha;
	long		diff;
...
//Writeln();
		...
		while (i < tm_cnt) {
			cha=(*tm_hit)[i].tm_cha;
//			if (inp_type[cha>>6] != F1TDC) { i++; continue; }

			if (cha >= 64*C_UNIT) break;
...
//
		...
		if (err) {
			...
			i=3;
			while (i < j) {
				printf(" %06lX", dmabuf[0][i] &0x3FFFFFL);
				i++;
			}
...
int pulse_width(void)
{
	...
	if (!pw_buf && ((pw_buf=malloc(sizeof(PW_BUF))) == 0)) {
		printf("can't allocate PW buffer\n");
		return CEN_PROMPT;
	}
	...
	while ((ret=read_event(EVF_NODAC, RO_RAW)) == 0) {
		...
		for (i=0; i < lx; i++) {
			...
			if (EV_FTIME(robf[i+s]) &1) {
				...
				pw_buf->ri_markl[cha]=(EV_FTIME(robf[i+s]) &~1);
				pw_buf->ri_markh[cha]=(u_char)EV_MTIME(robf[i+s]);
				continue;
			}

			fa_msk[cha>>5] |=(1L <<(cha &0x1F));
			pw_buf->fa_markl[cha]=EV_FTIME(robf[i+s]);
			pw_buf->fa_markh[cha]=(u_char)EV_MTIME(robf[i+s]);
		}
		...
		for (i=0, cha=0; cha < 64*C_UNIT; cha++) {
			...
			pw=pw_buf->fa_markl[cha]-pw_buf->ri_markl[cha];
			if (pw_buf->fa_markh[cha] != pw_buf->ri_markh[cha]) {
				if (pw_buf->fa_markh[cha] != (u_char)(pw_buf->ri_markh[cha]+1)) err=1;
