...
extern FQDCT_BC	*inp_bc;
extern FQDCT_RG	*inp_unit;
...
static int qdc_logdata(void)
{
	...
	inp_unit->cr=0;
	tmp=Q_ENA;
	if (sqd->bool &_RAW) tmp |=QFT_RAW;
	if (sqd->bool &_ITG) {	// internal trigger
		tmp |=Q_LTRIG;
		if (sqd->tlevel) tmp |=Q_LEVTRG;
	}

	if (sqd->bool &_EDG) tmp |=QFT_CLKPOL;
	if (sqd->bool &_POL) tmp |=QFT_ADCPOL;
	...
	dhdr.cr=tmp;
	dhdr.mean=inp_unit->mean_level;
	ret=inp_unit->mean_noise;
	delay(500);
	dhdr.noise=inp_unit->mean_noise;
	...
	if (i &QFT_ADCPOL) {
		for (i=hdr; i < ld; i++)
			fprintf(logfile, "%04X %04X\n",
					  (u_int)dmabuf[0][i]^0xFFF,
					  (u_int)dmabuf[0][i]);

		fprintf(logfile, "%04X %04X\n",
				  (u_int)dmabuf[0][ld+1]^0xFFF,
				  (u_int)dmabuf[0][ld+1]);
	} else {
		for (i=hdr; i < ld; i++)
			fprintf(logfile, "%04X\n",
	...
	while (1) {
		if ((ret=read_event(EVF_NODAC, RO_RAW)) != 0) {
			...
			if ((key >= '0') && (key <= '4')) {
				if (key == '4') {
					mx=3;
					inp_unit->cr=((tmp=inp_unit->cr) |QFT_SGRD);
				} else {
					mx=key-'0';
					inp_unit->cr=((tmp=inp_unit->cr) &~QFT_SGRD);
				}
...
if (li-ld != dq_len) {
	...
	continue;
			...
			imx=0; imn=0xFFFF;
			if (sqd->bool &_MEA) {
				tmp=(offl) ? (u_int)dmabuf[0][ld+1] : inp_unit->mean_level;
	...
	if (WaitKey('1')) return 0;
			...
			if (single == 2) {
				...
				if (key == 'A') {
					if (sqd->bool &_MEA) {
						u_int	tmp=(offl) ? (u_int)dmabuf[0][ld+1] : inp_unit->mean_level;
...
printf("Version                            %04X\n", inp_unit->ident);
				if (inp_conf->type != FQDCTST) {
					printf("this is not a QDC module!!!\n");
					return CE_NO_INPUT;
				}
			}

			if (inp_conf && (inp_conf->type == FQDCTST)) {
...
printf("configuration                      %04X ..2\n", inp_unit->mean_tol);
printf("trigger level                      %04X ..3\n", inp_unit->trig_level);
printf("latency                            %04X ..4\n", inp_unit->latency);
printf("window                             %04X ..5\n", inp_unit->window);

printf("           means =%03X, noise =%02X\n", inp_unit->mean_level, inp_unit->mean_noise);
printf("                              Mrst(1) %X ..7\n", inp_conf->slv_data[7]);
...
printf("JTAG                                    ..J\n");
		...
		if (inp_conf->type != FQDCTST) continue;
//
//---------------------------
//
		if (key == 0) {
		...
		if (key == 'J') {
			JTAG_menu(jt_inp_putcsr, jt_inp_getcsr, jt_inp_getdata, &config.jtag_qdc, 0);
			continue;
		}
