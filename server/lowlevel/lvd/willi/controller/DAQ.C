							...
							// 1: clear pages
{
	...
	for (j=0; j < 2*SZ_PAGE_L; dmadesc[j++]=0xCCCCCCCCL);
...
#define _ADB_DEL1	100
	...
	dmadesc[SZ_PAGE_L] =TEST_PAT+TEST_DESC+1;
	pg=0;
	tmp0=tmp1=0;
	do {
		...
		if ((pat &~(TEST_DESC-1)) == TEST_PAT) {
			...
			for (ux=0; ux < 2; ux++) {
				if (pat == TEST_PAT+TEST_DESC+ux) {
					dmadesc[ux*SZ_PAGE_L] =~(TEST_PAT+TEST_DESC+ux);
					plx_regs->dmacsr[1] =0x3;
					tmp=0;
					while (!(plx_regs->dmacsr[1] &0x10)) {
						_delay_(_ADB_DEL0);
...
static u_long	tmp;
	...
	if (!dmadesc) {
		//
		//--- allocate DMA buffer for descriptor table
		//
		...
		SetSelectorLimit(sel, (2*SZ_PAGE)-1);
...
static int	blk;
	...
	if (pg < 0) {
		...
		i=pg*SZ_PAGE_L; ln=0;
		while (i < 2*SZ_PAGE_L) {
			if (!(i &0x7)) printf("%04X: ", 4*i);

			printf(" %08lX", dmadesc[i++]);
			if (!(i &0x7)) {
				printf("\n");
				if (++ln >= 32) {
					if (WaitKey(0)) return CEN_PROMPT;
...
int bus_error(void)
{
	...
	if ((ret=protocol_error(-1, 0)) != 0) clr_prot_error();
...
if (!key) printf("%d\n", getch());
		...
		case '2':
				if (pci_ok == 2) ret=region_access_menu(reg_info, bus_error);
				break;
...
#if   (sizeof(F1TDC_RG) != 0x80) || (sizeof(F1TDC_BC) != 0x80)
	printf("F1TDC_RG:  %02X,  F1TDC_BC:  %02X\n", sizeof(F1TDC_RG), sizeof(F1TDC_BC));
	WaitKey(0);
#endif

#if   (sizeof(FQDCT_RG) != 0x80) || (sizeof(FQDCT_BC) != 0x80)
...
#if   (sizeof(COUPLER) != 0x80) || (sizeof(COUPLER_BC) != 0x80)
	printf("COUPLER:   %02X,  COUPLER_BC:%02X\n", sizeof(COUPLER), sizeof(COUPLER_BC));
	WaitKey(0);
#endif
