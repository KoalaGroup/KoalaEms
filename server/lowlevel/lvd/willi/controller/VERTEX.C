			...
			case 'V':   // LV & HV read cont
							{
								...
								while(1) {
									...
									while ((ret=read_event(EVF_NODAC, RO_RAW)) != 0) {
										printf("[KEINE] Daten gelesen... ret=%04X\n", ret);
										if (!(ret &CE_KEY_PRESSED)) break;

										c=toupper((u_char)ret);
										if (c == ESC) break;
...
int vertex_menu(void)
{
	...
	while (1) {
		...
		if (hd) {
		  ...
		  if (inp_conf && (inp_conf->type == VERTEX)) {
			if (inp_unit->ADR_ALLHIGH != 0xFFFF)
				printf("All High                   %04X ..3\n", inp_unit->ADR_ALLHIGH);
			if (inp_unit->ADR_ALLLOW != 0)
				printf("All LOW                    %04X ..4\n", inp_unit->ADR_ALLLOW);

			printf("Control                    %04X ..0\n", inp_unit->ADR_CTRL);
			printf("Status                     %04X\n", inp_unit->ADR_STAT);

			printf("Sequencer Control          %04X ..1\n", inp_unit->ADR_SEQCTRL);
			printf("Sequencer Status           %04X\n", inp_unit->ADR_SEQSTATE);
			printf("Trigger Offset             %04X ..2\n", inp_unit->ADR_TRIGGEROFFSET);
			printf("Slow Ctrl Clock /Command   %04X ..3/4\n", inp_unit->ADR_CKSREG);
			printf("Slow Ctrl Output           %04X\n", inp_unit->ADR_SCTRLOUT);
			printf("LV max Data Words          %04X ..5\n", inp_unit->ADR_MAX_DATA_LV);
			printf("LV Chip Mask               %04X ..6\n", inp_unit->ADR_CHIPMASK_LV);
			printf("LV ADC Offset              %04X ..7\n", inp_unit->ADR_ADCOFFSET_LV);
			printf("LV Sequender Polarity      %04X ..8\n", inp_unit->ADR_SEQPOL_LV);
			printf("LV Chip Select Polarity    %04X ..9\n", inp_unit->ADR_CSPOL_LV);
			printf("LV Software Trigger    %08lX ..a\n", inp_unit->ADR_SWREG_LV);
			printf("LV Counter  %04X/%04X/%04X/%04X\n",
				inp_unit->ADR_COUNTER_LV[0], inp_unit->ADR_COUNTER_LV[1], inp_unit->ADR_COUNTER_LV[2], inp_unit->ADR_COUNTER_LV[3]);
			printf("HV max Data Words          %04X ..b\n", inp_unit->ADR_MAX_DATA_HV);
			printf("LV/HV Serial Bus Polarity  %04X ..c\n", inp_unit->ADR_SERPOL);
			printf("HV Chip Mask               %04X ..d\n", inp_unit->ADR_CHIPMASK_HV);
			printf("HV ADC Offset              %04X ..e\n", inp_unit->ADR_ADCOFFSET_HV);
			printf("HV Sequender Polarity      %04X ..f\n", inp_unit->ADR_SEQPOL_HV);
			printf("HV Chip Select Polarity    %04X ..g\n", inp_unit->ADR_CSPOL_HV);
			printf("HV Software Trigger    %08lX ..H\n", inp_unit->ADR_SWREG_HV);
			printf("HV Counter  %04X/%04X/%04X/%04X\n",
				inp_unit->ADR_COUNTER_HV[0], inp_unit->ADR_COUNTER_HV[1], inp_unit->ADR_COUNTER_HV[2], inp_unit->ADR_COUNTER_HV[3]);
			printf("Memory/Sequencer Test/Seq Ctrl  ..t/S/s\n");
			printf("Slow Control/Read Data          ..o/i\n");
			printf("JTAG utilities                  ..j\n");
			printf("MASTER Reset/Clear Error        ..m/r\n");
		  }
			...
			if (rkey == ' ') rkey=UCLastKey[MEN_MAIN];

			if ((rkey == ESC) || (rkey == CR)) continue;
		}

		use_default=(rkey == TAB);
		if (use_default || (rkey == CR)) rkey=UCLastKey[MEN_MAIN];
		if (rkey > ' ') printf("%c", rkey);

		key=toupper(rkey);
		Writeln();
		if (rkey=='B') { inp_assign(sc_conf, 0); UCLastKey[MEN_MAIN]=key; continue; }
		...
		if (key == 'I') {
			u_int		ctrl=inp_unit->ADR_CTRL;
			u_int		cnt=0;
			u_char	dsp=0;
			u_int		l;
//			u_int		llen=0;

			printf("type 1..9 to display datablocks\n");
#ifdef PBUS
			inp_mode(IM_VERTEX);
#else
			config.daq_mode=EVF_HANDSH|MCR_DAQTRG;	// internal trigger
			inp_mode(0);	// IM_QUIET
			kbatt_msk=0;
#endif
			inp_unit->ADR_SWREG_LV=0xCCCC;
			inp_unit->ADR_SWREG_HV=0xCCCC;
			inp_unit->ADR_CTRL=ctrl;
			while (1) {
				if ((ret=read_event(EVF_NODAC, RO_RAW)) != 0) {
					...
					use_default=0;
					key=toupper((char)ret);
					if (key == ESC) { Writeln(); ret=CEN_PROMPT; break; }

					if ((key >= '1') && (key <= '9')) { dsp=key-'0'; clrscr(); }
					continue;
				}

				printf("\r%5u %04X", cnt++, rout_len);
#if 0
				if (rout_len != llen) {
					if (llen) Writeln();
					llen=rout_len;
		...
		i=key-'0';
		if (i > 9)
			if ((key >= 'A') && (key <= 'F')) i=i-7;
			else i=0xFFFF;

		if ((key == '4') || (key == 'A') || (key == 'H')) {
			if (key == '4') i=3;
			else
				if (key == 'A') i=2;
				else i=1;

			printf("Value 0x%08lX ", inp_conf->slv_xdata.l[i]);
			inp_conf->slv_xdata.l[i]=Read_Hexa(inp_conf->slv_xdata.l[i], -1);
			if (errno) continue;
			...
			if (key == 'G') i=4;
			if (i < 16) {
				printf("Value 0x%04X ", inp_conf->slv_data[i]);
				inp_conf->slv_data[i]=(u_int)Read_Hexa(inp_conf->slv_data[i], -1);
				if (errno) continue;
	...
	case 'R':	inp_unit->ADR_ACTION = CLEARERROR;
					break;

	case 'J':	JTAG_menu(jt_inp_putcsr, jt_inp_getcsr, jt_inp_getdata,&config.jtag_vertex, 1);
					break;

	case '0': 	inp_unit->ADR_CTRL=inp_conf->slv_data[i];
					break;

	case '1': 	inp_unit->ADR_SEQCTRL=inp_conf->slv_data[i];
					break;

	case '2': 	inp_unit->ADR_TRIGGEROFFSET=inp_conf->slv_data[i];
					break;

	case '3': 	inp_unit->ADR_CKSREG=inp_conf->slv_data[i];
					break;

	case '4':	inp_unit->ADR_SCTRL=inp_conf->slv_xdata.l[i];
					break;

	case '5': 	inp_unit->ADR_MAX_DATA_LV=inp_conf->slv_data[i];
					break;

	case '6': 	inp_unit->ADR_CHIPMASK_LV=inp_conf->slv_data[i];
					break;

	case '7': 	inp_unit->ADR_ADCOFFSET_LV=inp_conf->slv_data[i];
					break;

	case '8': 	inp_unit->ADR_SEQPOL_LV=inp_conf->slv_data[i];
					break;

	case '9': 	inp_unit->ADR_CSPOL_LV=inp_conf->slv_data[i];
					break;

	case 'A':	inp_unit->ADR_SWREG_LV=inp_conf->slv_xdata.l[i];
					break;

	case 'B': 	inp_unit->ADR_MAX_DATA_HV=inp_conf->slv_data[i];
					break;

	case 'C': 	inp_unit->ADR_SERPOL=inp_conf->slv_data[i];
					break;

	case 'D': 	inp_unit->ADR_CHIPMASK_HV=inp_conf->slv_data[i];
					break;

	case 'E': 	inp_unit->ADR_ADCOFFSET_HV=inp_conf->slv_data[i];
					break;

	case 'F': 	inp_unit->ADR_SEQPOL_HV=inp_conf->slv_data[i];
					break;

	case 'G': 	inp_unit->ADR_CSPOL_HV=inp_conf->slv_data[i];
					break;

	case 'H':	inp_unit->ADR_SWREG_HV=inp_conf->slv_xdata.l[i];
					break;

					// Memory Test
	case 'T':   UCLastKey[MEN_MAIN]=key;
					mem_menu();
					break;

					// SLOW Ctrl
	case 'O': 	slowctrl_menu();
					break;
	...
	case 'U': 	if (inp_assign(sc_conf, 2)) continue;
		...
		switch (rkey) {
	case 's':	seq_edit_menu((VTEX_RG*)inp_unit, LV);
					break;

	case 'S':	UCLastKey[MEN_MAIN]=key;	// Sequencer Test
					sequencer_menu();
					break;
		}
