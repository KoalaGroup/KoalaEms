...
extern unsigned int	*abuf[16];
extern char				UCLastKey[20];

#define ABUF(i)	(abuf[(u_int)((i) >> 15)])[(u_int)(i) &0x7FFF]
			...
			switch (hstr[0]) {
				...
				default:		// Datenzeile
					...
					if (mem==LV) {
						...
						inp_unit->ADR_RAM_DATA_LV=d;
					} else {
						inp_unit->ADR_RAM_ADR_HV=adr;
						inp_unit->ADR_RAM_DATA_HV=d;
					}
					adr++;
...
void WriteLVRAM(int mode) {
	long				l;
	unsigned int	d;

	randomize();
	switch (mode) {
		case 0 :	for (l=0; l<524288l; l++ ) {
						ABUF(l)=(unsigned int)(((unsigned int)random(256)<<8)|random(256));
					}
					break;

		case 1 :	for (l=0; l<262144l; l++ ) {
						ABUF(l)=(unsigned int)l;
					}
					for (l=262144l; l<524288l; l++ ) {
						ABUF(l)=(unsigned int)(262144l-l);
					}
					break;
	}
...
//	gotoxy(1,4);
	inp_unit->ADR_CTRL = 0x0001;
	inp_unit->ADR_CTRL = 0x0000;
	d					= 0;
	inp_unit->ADR_RAM_ADR_LV=0;

	for (l=0; l<524288l; l++) {
		inp_unit->ADR_RAM_ADR_LV=l;
		inp_unit->ADR_RAM_DATA_LV=ABUF(l); //d;
		if ((l%100)==0) if (protocol_error(-1, 0)) {getch(); return;  }
		if ((l%1000)==0) {
			if (protocol_error(-1, 0)) {getch(); return;}
			printf("\rWriting - ADR: %06ld",l,d); // , DATA: %04X"
...
//		inp_unit->ADR_RAM_ADR_LV=l;
		...
		if ((l%1000)==0) {
			printf("\rReading - ADR: %06ld",l); // , DATA: %04X, ERR: %ld
			if (kbhit()){
				c=toupper(getch());
				if ((c == CTL_C) || (c == ESC)) break;
			}
			if (s&&(l%32)==0) { while (!kbhit()); c=getch(); }
		}
	}
	printf("\n");
}
...
//		inp_unit->ADR_RAM_ADR_HV=l;
		...
		if ((l%1000)==0) {
			if (protocol_error(-1, 0)) return;
			printf("\rWriting - ADR: %06ld",l); // , DATA: %04X"
			if (kbhit()) { getch(); break; }
		}
	}
	printf("\n");
...
//			inp_assign(sc_conf, 1);
			...
			printf("\n");
			printf("\n");
			printf("LV RAM (random)            ..1\n");
			printf("LV RAM (rampe)             ..2\n");
			printf("HW RAM (random)            ..3\n");
			printf("HW RAM (rampe)             ..4\n");
			printf("LV & HW RAM (random)       ..5\n");
			printf("LV & HW RAM (rampe)        ..6\n");
			printf("WR-RD eine Zelle LV        ..7\n");
			printf("WR-RD eine Zelle HV        ..8\n");
			printf("\n");
			printf("MEM Dump LV RAM            ..A\n");
			printf("MEM Dump HV RAM            ..B\n");
...
//							clrscr();
							...
							printf("Writing...");
							while(!kbhit()) {
								inp_unit->ADR_RAM_ADR_LV=1;
								inp_unit->ADR_RAM_DATA_LV=d++;
								inp_unit->ADR_RAM_ADR_LV=0x00070001L;
								inp_unit->ADR_RAM_DATA_LV=d++;
								if (d>10000) d= 0;
								if ((d%1000)==0) {
									printf(".");
...
//		ux=key-'0';
		...
		switch (key) {
			...
			case '5':   // LV single
							inp_unit->ADR_CTRL = 0x0024;
							delay(1);
							inp_unit->ADR_CTRL = 0x002C;
							break;

			case '6': 	// LV cont
							inp_unit->ADR_CTRL   = 0x00A4;
							l=0;
...
//											 inp_unit->ADR_CTRL = cw;
			...
			case 'A':	inp_unit->ADR_ACTION = 0x0002;
			...
			case 'I':	// HV single
							inp_unit->ADR_CTRL = 0x0044;
							delay(1);
							inp_unit->ADR_CTRL = 0x0054;
							break;
			...
			case 'K':   // HV read single
							{
								...
								// HV freischalten
								inp_unit->ADR_CTRL = 0x00C4;
								// HV Trigger
								inp_unit->ADR_CTRL = 0x00D4;
			...
			case 'M':   // HV read cont
							{
								...
								// ADC Dummy Data, HV freischalten , Fast Mode (in line)
								inp_unit->ADR_CTRL = 0x00C4;
...
//									if (read_event()==0)
			...
			case 'O':	inp_unit->ADR_ACTION = 0x0004;
							printf("\nCounter zurueck gesetzt...\n\n");
							break;
			...
			case 'Q': 	// LV & HV single
							inp_unit->ADR_CTRL   = 0x00E4;
							delay(10);
							inp_unit->ADR_CTRL = 0x00FC;
							break;
...
//									if (read_event()==0)
			...
			case 'X':	inp_unit->ADR_ACTION = 0x0002;
							inp_unit->ADR_ACTION = 0x0004;
							printf("\nCounter zurueck gesetzt...\n\n");
							break;
		...
		switch (key) {
			...
			case '2': 	{
								...
								while (1) {
									...
									//Poti beschreiben
									l = lpot;
									inp_unit->ADR_SCTRL = 0x00007801 | (u_long)(l<<24);
