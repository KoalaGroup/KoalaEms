...
//static char		m_ok=0;
extern u_long		*abuf[];
#define ABUF(i)	(abuf[(u_int)(i) >> 14])[(u_int)(i) &0x3FFF]
...
"--------------------------------------------------------------------------------");
		...
		for (mx=0, i=prt_org, n=0; n < PRT_X; i++, n++)
			if (mx < ABUF(i)) mx =ABUF(i);
	}

	if (!mx) return;

	for (i=prt_org, n=0; n < PRT_X; i++, n++)
		dis[n]=(u_int)(((PRT_Y-1)*ABUF(i))/mx);
...
static int F1_bin_distribution(void)
{
	...
	while (1) {
		...
		if (mt && (rout_hdr &MCR_DAQTRG)) ABUF(rout_trg_time)++;

		i=h; lx =rout_len >>2;
		while (i < lx) {
			cha=EV_CHAN(robf[i]);
			if (cmask[cha >>5] &(1L <<(cha &0x1F))) ABUF(EV_FTIME(robf[i]))++;
	...
	do {
		...
		if (!(i &1)) {
			wth0 +=ABUF(i);
			if (mx0 < ABUF(i)) { mx0 =ABUF(i); m0=i; }
			continue;
		}

		wth1 +=ABUF(i);
		if (mx1 < ABUF(i)) { mx1 =ABUF(i); m1=i; }
	...
	if (i) {
		...
		printf("0x%04X:\n", i);
		while (1) {
			v0=(u_int)((79L*ABUF(i))/mx); i++;
			v1=(u_int)((79L*ABUF(i))/mx);
			c=0;
			do {
				if (v0 == c) {
					if (v0 == v1) { printf("Û"); c++; break; }
...
#define LPHP	49
	...
	fprintf(logfile, "0x%04X:\n", s);
	do {
		v0=(u_int)(((u_long)CPL*ABUF(i))/mx); i++;
		v1=(u_int)(((u_long)CPL*ABUF(i))/mx); i++;
		c=0;
		do {
			if (v0 == c) {
...
static int save_distribution(void)
{
	...
	for (i=0; i < 8; i++)
		if ((u_int)write(hdl, abuf[i >>1] +((i&1)<<13), 0x8000) != 0x8000) {
...
"--------------------------------------------------------------------------------");
		...
		if (!(lp &0x0FF)) {
			...
			for (i=0; i < 78;) {
				tmp=ABUF(prt_org+i);
				if (  ((i == 0) || (ABUF(prt_org+i-1) <= tmp))
					 && (ABUF(prt_org+i+1) < tmp)) {
...
static int	blk;
	...
	if (*str) {
		...
		for (i=0; i < 8; i++)
			if ((u_int)read(hdl, abuf[i >>1] +((i&1)<<13), 0x8000) != 0x8000) {
...
int measurement(void)
{
	...
	if (!m_ok) {
		for (i=0; i < 4; i++)
			if (!abuf[i] && ((abuf[i]=calloc(0x8000, 2)) == 0)) {
