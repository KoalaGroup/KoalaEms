...
static int sram_test(void)
{
	...
	if (config.srtst_end > 2*SQ_SRAM_LEN) config.srtst_end=2*SQ_SRAM_LEN;
	strt=config.srtst_strt;
	end =config.srtst_end;
	if (end == 0) end=2*SQ_SRAM_LEN;
	...
	if (lm) lm--;
	dout=0; din=0;
	inp_unit->ADR_SRAM_ADR=strt;
	for (i=0; i < end; i++) {
		inp_unit->ADR_SRAM_DATA=dout;
		dout +=_PTEST_INC;
...
#define _BQCNT_		((4*SQ_SRAM_LEN)/_BLEN_)
...
static int sram_dtest(void)
{
	...
	if (dmapages < 0x100) {
		printf("it will use min 0x100 DMA pages\n");
	...
	setup_dma(1, 0, 0, _BLEN_, (u_int)&inp_unit->ADR_SRAM_DATA, 0, 0);	// for SRAM write
...
//if (gigal_regs->balance) printf(" balance\n");
		...
		if (mode&2) {
			...
			for (blk=0; blk < _BQCNT_; blk++) {
				...
				gigal_regs->t_ad =(u_int)&inp_unit->ADR_SRAM_DATA;
#if (_BLEN_ > 0x1000L)
		...
		if (kbhit()) {
			...
			sec=tm /100.0;
			mb =1.0*lp*(4*SQ_SRAM_LEN)/1000000.;
			if (mode == 3) mb *=2.;
...
//--------------------------- t_sram_test -----------------------------------
//
#define TB_SRAM_LEN	0x00200000L		// SRAM word

static int t_sram_test(void)
{
	u_short	dout,din;
	u_long	x;
	u_long	lp,lm;
	...
	printf("start address  %06lX ", config.trtst_strt);
	config.trtst_strt=Read_Hexa(config.trtst_strt, -1);
	if (errno) return CEN_MENU;

	printf("end address    %06lX ", config.trtst_end);
	config.trtst_end=Read_Hexa(config.trtst_end, -1);
	if (errno) return CEN_MENU;

	if (config.trtst_end > TB_SRAM_LEN) config.trtst_end=TB_SRAM_LEN;
	strt=config.trtst_strt;
	end =config.trtst_end;
	if (end == 0) end=TB_SRAM_LEN;
	...
	if (lm) lm--;
	dout=0; din=0;
	inp_unit->ADR_RAMT_ADR=strt;
	for (i=0; i < end; i++) {
		inp_unit->ADR_RAMT_DATA=dout;
		dout +=_PTEST_INC;
		if ((u_short)i == 0xFFFF) dout +=_PTEST_INC;
	}
...
//--------------------------- t_sram_dtest ----------------------------------
//
#define _BTCNT_		((2*TB_SRAM_LEN)/_BLEN_)

static int t_sram_dtest(void)
{
	u_int		blk;
	u_long	dout,din;
	u_long	x,y;
	...
	printf("mode (rd:2, wr:1) %3u ", config.trtst_mode);
	config.trtst_mode=(u_int)Read_Deci(config.trtst_mode, 3);
	mode=config.trtst_mode &3;
	if (!mode || errno) return CEN_MENU;

	setup_dma(1, 0, 0, _BLEN_, (u_int)&inp_unit->ADR_RAMT_DATA, 0, 0);	// for SRAM write
	setup_dma(-1, 0x100, 0, _BLEN_, 0, 0, DMA_L2PCI);							// for SRAM read
	lp=0;
	dout=0; din=0;
	if (!(mode&2)) {
	...
	while (1) {
		...
		if (mode&2) {
			...
			for (blk=0; blk < _BTCNT_; blk++) {
				gigal_regs->d0_bc=0;
				gigal_regs->t_hdr=HW_BE|HW_L_DMD|HW_R_BUS|HW_EOT|HW_FIFO|HW_BT|THW_SO_DA;
				gigal_regs->t_ad =(u_int)&inp_unit->ADR_RAMT_DATA;
#if (_BLEN_ > 0x1000L)
				plx_regs->dma[0].dmadpr=dmadesc_phy[1] |0x9;
#endif
				plx_regs->dmacsr[0]=0x3;								// start DMA
		...
		if (kbhit()) {
			...
			sec=tm /100.0;
			mb =1.0*lp*(2*TB_SRAM_LEN)/1000000.;
			if (mode == 3) mb *=2.;
			printf("time = %4.2f sec, one loop = %4.2f sec\n", sec, sec/lp);
			printf("Rate = %1.3f MByte per sec\n", mb /sec);
			return CE_PROMPT;
	...
	int		hv)
{
	...
	while (1) {
		...
		while (1) {
			...
			if ((rkey == TAB) || (rkey == CR)) rkey=config.i3_menu;
			...
			while (kbhit()) getch();
			if ((ret == CE_MENU) || (ret == CE_PROMPT)) config.i3_menu=rkey;
