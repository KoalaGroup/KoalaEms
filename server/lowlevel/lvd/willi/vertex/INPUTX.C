...
{
	u_long	sz=phypages*(u_long)SZ_PAGE;
	u_long	lx;
	u_int		i,l;
	char		hd=-1;
...
static u_long	dblen=4;
	...
	setup_dma(-1, 0, 0, phypages*(u_long)SZ_PAGE, 0, 0, DMA_L2PCI);
	...
	while (1) {
      ...
      if (hd == 1) {
		...
		l=(u_int)(lx >>2); c=0;
		for (i=0; i < l; i++) {
			if (!sl && ((u_int)(dmabuf[i] >>16) == 0x8000)) {
				if (c) { ln++; Writeln(); c=0; }
...
//printf("%08lX %08lX\n", gigal_regs->d0_bc, cm_reg->dd_counter);
				...
				if (ddat) {
					lx=(u_int)dmabuf[dd_ix]>>2;
					if (((dmabuf[dd_ix] >>16) != 0x8000) || (lx < 3)) {
						printf("format error\n");
						return CE_PROMPT;
					}
				...
				if (lerr && (lerr != LPE_DLOCK) && (lerr != LPE_RESOURCE)) {
					cm_reg->cr =0;
					gigal_regs->d0_bc_adr =dmabuf_phy[phypages-1] |0x1;
...
// DMA read block
//---------------
				setup_dma(-1, 0, 0, phypages*(u_long)SZ_PAGE, 0, 0,
							 (rout_hdr) ? DMA_L2PCI
...
//								BUS_ERROR;
						...
						gigal_regs->t_da =phypages*(u_long)SZ_PAGE -j;
						gigal_regs->t_ad=(u_int)&((F1TDC_RG*)iconf->inp_regs)->ro_data;
					}
...
//printf("inf/d0_bc: %08lX/%04X\n", rout_ev_inf, rout_len);
						...
						while (1) {
							...
							if (!(u_int)dmabuf[i] || (j > rout_len)) {
								if ((long)glp != -1) printf("\r%10ld: ", glp);
								printf("length err, blk=%u, len=%04X/%04X, h=%08lX, i=%04X/%04X\n",
										 h-1, rout_len, j, dmabuf[i], i, 4*i);
								return CE_FATAL;
							}
...
//				if (config.daq_mode &EVF_NOBCBUF) config.daq_mode |=EVF_DDATA;
		...
		if (config.daq_mode &(EVF_NOBCBUF|EVF_DDATA)) {
			...
			if (config.dd_block >= ((phypages-1)*(u_long)SZ_PAGE-0x800))
				printf("attention max is %06lX\n", (phypages-1)*(u_long)SZ_PAGE);
		}
#endif
...
#ifdef SC_M
	if ((config.daq_mode &(EVF_NOBCBUF|EVF_DDATA)) ? (phypages < 5)
												  : (phypages < 2)) return CE_NO_DMA_BUF;
#else
	if (phypages < 2) return CE_NO_DMA_BUF;
#endif
...
//printf("sr     =%08lX\n", ltmp);
			...
			while (1) {
				...
				if ((j=i+1) >= dd_max) j=0;
				if ((k=j+1) >= dd_max) k=0;
				stn=(u_char)(dmabuf[j] >>16);
				if (stn == 15) stn=0; else stn++;

				if (stn >= 6) {
					printf("\r%10lu hdr error %08lX/%08lX/%08lX, i=%04X/%04X\n",
							glp, dmabuf[i], dmabuf[j], dmabuf[k], i, 4*i);
					ret=CE_PROMPT; break;
				}

				if ((tmp != (u_int)evlen[stn]) || (dmabuf[k] != evcounter[stn])) {
					if (!pciconf.errlog) {
						evlen[stn]=dmabuf[i];
						evcounter[stn]=dmabuf[k];
					} else {
						if (!evlen[stn]) evlen[stn]=dmabuf[i];
						else {
							printf("\r%10lu\n", glp);
							printf("error(%u), h=%08lX/%08lX/%08lX (%08lX/%08lX), i=%04X/%04X\n",
									 stn, dmabuf[i], dmabuf[j], dmabuf[k],
									 evlen[stn], evcounter[stn], i, 4*i);
							ret=CE_PROMPT; break;
						}
					}
