...
#define CONLD		0xF0	// so genannt in SVF
...
static DEV_DESC	dev_desc[]={
	{0x05024093l, "XC18V01 ", 8},
	{0x05025093l, "XC18V02 ", 8},
	{0x05026093l, "XC18V04 ", 8},
	{0x05036093l, "XC18V04 ", 8},
	{0x05044093l, "XCF01S  ", 8},
	{0x00618093l, "XCV150  ", 5},
	{0x00620093l, "XCV300  ", 5},
	{0x00628093l, "XCV400  ", 5},
	{0x01038093l, "XC2V2000", 6},
	{0x01414093l, "XC3S200 ", 6},
	{0}
};
...
//
	dout=jtag_data(-1, 32); erasetest=1500000L;
	if ((dout &0x0FFFFFFFL) == 0x05024093L) {			// XC18V01 1 Mbit
		...
		if ((dout &0x0FFFFFFFL) == 0x05025093L) {		// XC18V02 2 Mbit
			...
			if ((dout &0x0FFFFFFFL) == 0x05026093L) {	// XC18V04 4 Mbit
				...
				if ((dout &0x0FFFFFFFL) == 0x05036093L) {	// XC18V04 neu
					data0=0x200; size0=0x80000L; erasetest=15000000L;
				} else {
					if ((dout &0x0FFFFFFFL) == 0x05044093L) {	// XCF01S 1 Mbit
						data0=0x100; size0=0x80000L; erasetest=15000000L;
					} else {
...
//		if (ret != 0x01) printf("BYPASS.0 %02X\n", ret);
		...
		do {
			...
			lx +=ux;
			while (ux < data0) jtbuf->dr_tdi[ux++] =0xFF;

			jtag_instruction(FDATA0);
			if ((ret=jtag_wr_data(jtbuf->dr_tdi, data0, -1)) != 0) return ret;

			if (lx <= data0) {	// 1. mal
				printf("loading with 'faddr' instruction (16/0x0000)\n");
				ret=jtag_instruction(FADDR);
...
// sc_command, sc_length, lvir_tdi, lvir_tdo, lvir_mask);
				...
				if (tdo_val && ((ret ^lvir_tdo) &lvir_mask)) {
					if (!(~((1 <<jtag_tab.devs[jtag_tab.select].ir_len) -1)
							&lvir_mask)) {
						if (pt) { Writeln(); pt=0; }
