...
static DEV_DESC	dev_desc[]={
	{0x05024093L, "XC18V01 ", FM_18V, 8, 0x100, 0x020000L, 1500000L},
...
// 10000 => 1ms
	cnt /=10000L;
	if (cnt < 3000) { delay((u_int)cnt +1); return 0; }
					...
					// 2: read and save
{
	...
	if (mode == 2) {
		...
		if ((mode == 0) && (ddesc->fam &(FM_18V|FM_F_P))) {
			if (ddesc->fam &FM_18V) {
				jtag_instruction(USERCODE);
			} else {
				if (ddesc->fam &FM_F_P) {
...
//
		if (ddesc->fam &(FM_F_S|FM_18V)) {
			...
			if (ddesc->fam &FM_F_S) {
				...
				printf("loading with 'fdata3' instruction (3/0x07)\n");
				...
				printf("loading with 'fvfy3' instruction (3/0x07)\n");
				jtag_instruction(FVFY3);
				jtag_runtest(50);
				jtag_data(0x7, 3);

				printf("loading with 'conld' instruction\n");
				jtag_instruction(CONLD);
				jtag_runtest(110000L);
			...
			if (jt_usercode != 0xFFFFFFFFL) {
				...
				ret=jtag_instruction(USERCODE);
				jtag_data(jt_usercode, 32);

				jtag_instruction(FPGM);
				jtag_runtest(14000);

				ret=jtag_instruction(FVFY6);
				jtag_runtest(50);
