...
//
// $Id: libpcan.c,v 1.1 2007/03/02 22:52:47 wuestner Exp $
//
//****************************************************************************
...
#ifdef XENOMAI
    if ((err = rt_dev_ioctl(desc->nFileNo, PCAN_GET_STATUS, &status)) < 0)
#else
...
#ifdef XENOMAI
    if ((err = rt_dev_ioctl(desc->nFileNo, PCAN_READ_MSG, &m)) < 0)
#else
...
DWORD LINUX_CAN_Read(HANDLE hHandle, TPCANRdMsg* pMsgBuff)
{
  ...
  if (desc)
#ifdef XENOMAI
    err = rt_dev_ioctl(desc->nFileNo, PCAN_READ_MSG, pMsgBuff);
#else
...
DWORD LINUX_CAN_Statistics(HANDLE hHandle, TPDIAG *diag)
{
  ...
  if (desc)
#ifdef XENOMAI
    err = rt_dev_ioctl(desc->nFileNo, PCAN_DIAG, diag);
#else
...
WORD LINUX_CAN_BTR0BTR1(HANDLE hHandle, DWORD dwBitRate)
{
  ...
  if (desc)
#ifdef XENOMAI
    err = rt_dev_ioctl(desc->nFileNo, PCAN_BTR0BTR1, &ratix);
#else
...
#ifdef XENOMAI
    if ((err = rt_dev_ioctl(desc->nFileNo, PCAN_GET_EXT_STATUS, &status)) < 0)
#else
