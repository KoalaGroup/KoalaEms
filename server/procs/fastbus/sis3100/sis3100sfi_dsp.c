...
static const char* cvsid __attribute__((unused))=
    "$ZEL: sis3100sfi_dsp.c,v 1.5 2017/10/09 21:25:37 wuestner Exp $";
...
check_dsp_present(int crate)
{
    ...
    dsp_procs=sfi->get_dsp_procs(dev);
    if (!dsp_procs) {
...
check_mem_present(int crate)
{
    ...
    if (dev->fastbustype!=fastbus_sis3100_sfi)
        return BadModTyp;
    sfi=&dev->info.sis3100_sfi;
    mem_procs=sfi->get_mem_procs(dev);
    if (!mem_procs) {
        printf("sis3100dsp(fb)...: invalid controller\n");
        return plErr_BadModTyp;
    }
...
dump_mem(struct fastbus_dev* dev, int start, int size, const char* format)
{
    ...
    int l, n, i;
    struct mem_procs* mem_procs=dev->info.sis3100_sfi.get_mem_procs(dev);
...
/*****************************************************************************/
char name_proc_sis3100sfi_dsp_reset[] = "sis3100dsp_reset";
int ver_proc_sis3100sfi_dsp_reset = 1;
/*
 * p[0]: argcount==1
 * p[1]: crate ID
 */
plerrcode proc_sis3100sfi_dsp_reset(ems_u32* p)
{
    struct fastbus_dev* dev=get_fbdevice(p[1]);
    struct dsp_procs* dsp_procs=dev->info.sis3100_sfi.get_dsp_procs(dev);

    dsp_procs->reset(dev);
    return plOK;
}

plerrcode test_proc_sis3100sfi_dsp_reset(ems_u32* p)
{
...
/*****************************************************************************/
char name_proc_sis3100sfi_dsp_start[] = "sis3100dsp_start";
int ver_proc_sis3100sfi_dsp_start = 1;
/*
 * p[0]: argcount==1
 * p[1]: crate ID
 */
plerrcode proc_sis3100sfi_dsp_start(ems_u32* p)
{
    struct fastbus_dev* dev=get_fbdevice(p[1]);
    struct dsp_procs* dsp_procs=dev->info.sis3100_sfi.get_dsp_procs(dev);
...
plerrcode test_proc_sis3100sfi_dsp_start(ems_u32* p)
{
    if (p[0]!=1) return plErr_ArgNum;
    wirbrauchen=0;
    return check_dsp_present(p[1]);
...
/*****************************************************************************/
char name_proc_sis3100sfi_dsp_load[] = "sis3100dsp_load";
int ver_proc_sis3100sfi_dsp_load = 1;
/*
 * p[0]: argcount
 * p[1]: crate ID
 * p[2...]: filename
 */
plerrcode proc_sis3100sfi_dsp_load(ems_u32* p)
{
    struct fastbus_dev* dev=get_fbdevice(p[1]);
    struct dsp_procs* dsp_procs=dev->info.sis3100_sfi.get_dsp_procs(dev);
    char* filename;
...
plerrcode test_proc_sis3100sfi_dsp_load(ems_u32* p)
{
    plerrcode pres;
...
plerrcode proc_sis3100sfi_dsp_init(ems_u32* p)
{
    ...
    u_int32_t list[2];
    u_int32_t liststart, descrstart, descrentry;
    int procid, modultype, i;
    ...
    case PH_ADC_10C2:
        ...
        descr_block.extra.ph10CX.pa=module->address.fastbus.pa;
        descr_block.extra.ph10CX.num=p[4];
        break;
    case LC_ADC_1881:
        if (p[3]!=3) return plErr_ArgNum;
        descr_block.extra.lc1881.pa=module->address.fastbus.pa;
        descr_block.extra.lc1881.pat=p[4];
        descr_block.extra.lc1881.bc=p[4];
        descr_block.extra.lc1881.num=p[4];
        break;
    case LC_TDC_1877:
        break;
    case LC_TDC_1875A:
    ...
    mem_procs->write(dev, list, sizeof(list), liststart+8*p[1]);
