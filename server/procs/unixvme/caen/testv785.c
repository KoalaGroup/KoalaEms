...
static ems_u32 mtypes[]={CAEN_V785, 0};
...
{ //printf("proc_v792reset is OK1\n");
    int i, pres=plOK;
//printf("proc_v792reset is OK2\n");
    if (p[0] && (p[1]>-1)) {
//printf("proc_v792reset is OK3\n");
        pres=v792reset_module(p[1]);
// printf("proc_v792reset is OK4\n");
    } else {
        for (i=memberlist[0]; i>0; i--) {
//printf("proc_v792reset: OK44\n");
            pres=v792reset_module(i);
//printf("proc_v792reset: pres is=%d\n",pres);
            if (pres!=plOK)
                break;
//printf("proc_v792reset: OK5\n");
        }
    }
    return pres;
}
...
plerrcode test_proc_v792reset(ems_u32* p)
{
    ...
    if ((p[0]!=0) && (p[0]!=1))
        return plErr_ArgNum;
  //   printf("test_proc_v792reset: OK6\n");
  //   printf("memberlist=%p, mtypes=%p\n",memberlist,mtypes);
    if ((res=test_proc_vme(memberlist, mtypes))!=plOK)
printf("test_proc_v792reset: test_proc_vme failed!,return res=%d\n",res);
    //    return plOK;
        return res;
  ...
  //   printf("res is: %d\n",res);
    ...
    wirbrauchen=0;
printf("test_proc_v792reset  OK8\n");
    return plOK;
...
printf("data is:0x%d\n",data);
printf("proc_v792read: res1 is %d\n", res);

        if (res!=4) return plErr_System;
        code=(data>>24)&7;
printf("proc_v792read: code is 0x%d\n", code);
...
//printf("proc_v792read: buffer is not empty\n");
        if ((code=(data>>5)&7)!=2) { /* no header */
        ...
        *outptr++=data;
        count=(data>>8)&0x3f;
printf("proc_v792read: count is %d\n",count);
 ...
 * p[0]: argcount==0
 */
plerrcode proc_v792read_test(ems_u32* p)
{
    int i, res;
...
plerrcode test_proc_v792read_test(ems_u32* p)
{
    plerrcode res;
...
char name_proc_v792read_test[] = "v792read_test";
int ver_proc_v792read_test = 1;
