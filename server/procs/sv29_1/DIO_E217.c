#include <errorcodes.h>                   /* dieser wird immer gebraucht: */
#include <types.h>
#include <stdio.h>
#include <modultypes.h>                   /* dieser wird immer gebraucht: */  
#include <sconf.h>                       /* dieser wird immer gebraucht: */
#include <debug.h>
#include "../../OBJECTS/VAR/variables.h"  /* dieser wird immer gebraucht: */
#include "../../LOWLEVEL/VME/vme.h"       /* dieser wird immer gebraucht: */
...
D(D_REQ,printf("DIO_E217_in Modul p[0]=%d p[1]=%d p[2]=%d \n",p[0],p[1],p[2]);)
adresse=memberlist[welcher];
D(D_REQ,printf("DIO_E217_in Modul adresse =%d %X \n",adresse,adresse);)
value  =p[2];
...
D(D_REQ,printf("DIO_E217_in test modulnummer =%d %X \n",get_mod_type(memberlist[p[1]]),get_mod_type(memberlist[p[1]]));)
...
D(D_REQ,printf("DIO_E217_out Modul %d\n",p[1]);)
...
D(D_REQ,printf("DIO_E217_out var_list[p[2]].var.val %d %X\n",var_list[p[2]].var.val);)
