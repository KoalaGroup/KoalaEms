/*
 * ems/events/syncstatist3.c
 * 
 * 2009-Feb-18 PW
 * 
 * $ZEL: syncstatist3.c,v 1.1 2010/09/10 22:38:32 wuestner Exp $
 */
...
struct lvddecoder {
    int addr;
    int (*func)(FILE *po, struct subevent *sev, int idx,
            int silent, const char *name, int *tdt, int *acc, int *rej);
    const char *name;
};
struct decoder {
    int ID;
...
/******************************************************************************/
static int
decode_ignore(FILE *po, struct subevent *sev, struct subevent_queue *queue,
        void *aux)
{
    return 0;
}
...
/******************************************************************************/
static int
subdecode_ZEL_SYNC_O(FILE *po, struct subevent *sev, int idx, int silent,
    const char *name, int *tdt_, int *acc_, int *rej_)
{
    u_int32_t d=sev->data[idx];
    int addr=(d>>28)&0xf;

    if (!silent) {
        fprintf(po, "  %4d %08x %x %s", idx, d, addr, name);
    }

    if (d&0x40000) { /* is event count */
        u_int32_t evc=d&0xffff;
        if (silent) {
            if (evc!=(sev->evno-1)&0xffff) {
                fprintf(po, "wrong event number in ZEL_SYNC_O addr %x\n", addr);
                return -1;
            }
        } else {
            fprintf(po, " evc=%u\n", evc);
        ...
        if (silent) {
            if (!err && !tout) {
                add_ldt(sev->evno-1, addr, chan, deadtime);
        } else {
            double ftime;
            ftime=deadtime*100.;
            fprintf(po, " chan %x dt %f ns", chan, ftime);
            if (err)
                fprintf(po, ", error");
            if (tout)
                fprintf(po, ", timeout");
...
/******************************************************************************/
static double last_ev_time=0;
static double curr_ev_time=0;
static double last_acc_time=0;
static double curr_acc_time=0;
static double last_corr_acc=0;
static double curr_corr_acc=0;
static double old_fh1h=0;
static int
subdecode_ZEL_SYNC_M(FILE *po, struct subevent *sev, int idx, int silent,
    const char *name, int *tdt_, int *acc_, int *rej_)
{
    u_int32_t d=sev->data[idx];
    int addr=(d>>28)&0xf;
    int time, trigg, reject, tdt;
    double ftime;
    ...
    if (tdt) {
        ftime=time*100.;
        if (!silent)
            fprintf(po, " tdt %7u %f ns\n", time, ftime);
        add_tdt(sev->data[1], time);
        if (tdt_)
            *tdt_=1;
    } else {
        ftime=time*12.5;
        ...
        if (reject) {
            if (rej_)
                *rej_=1;
            if (!silent) {
                fprintf(po, " rej: trigg %x %7u %f ns\n",
                       trigg, time, ftime);
            ...
            if (acc_)
                *acc_=1;
            if (silent) {
                double ev_trg, ev_trg_mult, ev_trg_rem;
                last_acc_time=curr_acc_time;
                curr_acc_time=ftime;
                ev_trg=curr_ev_time-curr_acc_time;
                ev_trg_mult=trunc(ev_trg/13107200.0);
                ev_trg_rem=ev_trg-ev_trg_mult*13107200.0;
                last_corr_acc=curr_corr_acc;
                curr_corr_acc=curr_ev_time+ev_trg_rem;
            } else {
                fprintf(po, " acc: trigg %x %7u %f ns\n",
                        trigg, time, ftime);
            }
        }
        //add_trigger(sev->data[1], time, trigg, reject)
    }
    return trigg;
}
/******************************************************************************/
static int
subdecode_ZEL_F1(FILE *po, struct subevent *sev, int idx, int silent,
    const char *name, int *tdt, int *acc, int *rej)
{
    u_int32_t d=sev->data[idx];
    int addr=(d>>28)&0xf;
...
/******************************************************************************/
static int
subdecode_ZEL_empty(FILE *po, struct subevent *sev, int idx, int silent,
    const char *name, int *tdt, int *acc, int *rej)
{
    u_int32_t d=sev->data[idx];
    int addr=(d>>28)&0xf;
...
/******************************************************************************/
static int
decode_ZEL_MIX(FILE *po, struct subevent *sev, struct subevent_queue *queue,
        void *aux)
{
    struct lvddecoder *lvddecoder=(struct lvddecoder*)aux;
    u_int32_t /*wordcount,*/ h0, h1, h2, bytes;
    int idx=0, addr, fragment, xres=0, res=0;
    int tdt, acc, rej;
    int tdt_, acc_, rej_;
    double fh1h, fh1l, etime;

    if (sev->size<3) {
        fprintf(po, "E subevent too short\n");
        for (; idx<sev->size; idx++) {
            fprintf(po, "  %4d %08x", idx, sev->data[idx]);
        }
        return -1;
    }

#if 0
    wordcount=sev->data[idx++];
    ...
    fh1l=(h1&0xffff)*.1203;
    etime=fh1h+fh1l;
    last_ev_time=curr_ev_time;
    while (etime<last_ev_time)
        etime+=0xffff*6400.;
    curr_ev_time=etime;
...
#endif
#if 0
    fprintf(po, "  %4d %08x head_1 (trigger time) %d %d %f\n",
            2, h1, (h1>>16)&0xffff, h1&0xffff, ttime);
#endif
#if 0
    ...
    for (; idx<sev->size; idx++) {
        tdt_=acc_=rej_=0;
        addr=(sev->data[idx]>>28)&0xf;
        xres=lvddecoder[addr].func(po, sev, idx, 1, lvddecoder[addr].name,
                &tdt_, &acc_, &rej_);
        tdt+=tdt_;
        acc+=acc_;
        rej+=rej_;
    ...
    //if (h2>1000 && !acc) {
    //if (tdt>0) {
        double ev_diff, acc_diff;
        double ev_trg, ev_trg_mult, ev_trg_rem;
        fprintf(po, "event %d tdt=%d acc=%d rej=%d\n", h2, tdt, acc, rej);
        for (idx=3; idx<sev->size; idx++) {
            addr=(sev->data[idx]>>28)&0xf;
            if (lvddecoder[addr].func!=subdecode_ZEL_F1) {
                lvddecoder[addr].func(po, sev, idx, 0, lvddecoder[addr].name,
                        0, 0, 0);
            }
        }

        ev_diff=curr_ev_time-last_ev_time;
        acc_diff=curr_acc_time-last_acc_time;
        ev_trg=curr_ev_time-curr_acc_time;
        ev_trg_mult=trunc(ev_trg/13107200.0);
        ev_trg_rem=ev_trg-ev_trg_mult*13107200.0;

        if (fh1h<old_fh1h)
            fprintf(po, "xxxxxxxxxx\n");
        fprintf(po, "fh1h=%f\n", fh1h);
        fprintf(po, "diff between events: %f ns\n", ev_diff);
        fprintf(po, "diff between triggs: %f ns\n", acc_diff);
        fprintf(po, "diff between triggs: %f ns (corr)\n", curr_corr_acc-last_corr_acc);
        fprintf(po, "diff between event and trig: %f ns\n", curr_ev_time-curr_acc_time);
        fprintf(po, "diff between diffs: %f ns\n", ev_diff-acc_diff);
        fprintf(po, "split diff: %.1f %f ns\n", ev_trg_mult, ev_trg_rem);
        old_fh1h=fh1h;
    }
...
struct lvddecoder subdeco_sync_anke[]={
    {0x0, subdecode_ZEL_F1, "F1"},
    {0x1, subdecode_ZEL_F1, "F1"},
    {0x2, subdecode_ZEL_F1, "F1"},
    {0x3, subdecode_ZEL_F1, "F1"},
    {0x4, subdecode_ZEL_F1, "F1"},
    {0x5, subdecode_ZEL_F1, "F1"},
    {0x6, subdecode_ZEL_F1, "F1"},
    {0x7, subdecode_ZEL_F1, "F1"},
    {0x8, subdecode_ZEL_empty, "illegal"},
    {0x9, subdecode_ZEL_empty, "illegal"},
    {0xa, subdecode_ZEL_empty, "illegal"},
    {0xb, subdecode_ZEL_empty, "illegal"},
    {0xc, subdecode_ZEL_SYNC_O, "SYNC_O"},
    {0xd, subdecode_ZEL_SYNC_O, "SYNC_O"},
    {0xe, subdecode_ZEL_SYNC_O, "SYNC_O"},
    {0xf, subdecode_ZEL_SYNC_M, "SYNC_M"}
};

struct lvddecoder subdeco_sync_tof[]={
    {0x0, subdecode_ZEL_empty, "illegal"},
    {0x1, subdecode_ZEL_empty, "illegal"},
    {0x2, subdecode_ZEL_empty, "illegal"},
    {0x3, subdecode_ZEL_empty, "illegal"},
    {0x4, subdecode_ZEL_empty, "illegal"},
    {0x5, subdecode_ZEL_empty, "illegal"},
    {0x6, subdecode_ZEL_empty, "illegal"},
    {0x7, subdecode_ZEL_empty, "illegal"},
    {0x8, subdecode_ZEL_empty, "illegal"},
    {0x9, subdecode_ZEL_empty, "illegal"},
    {0xa, subdecode_ZEL_empty, "illegal"},
    {0xb, subdecode_ZEL_empty, "illegal"},
...
struct decoder decolist_tof[]={
    ...
    {1000, decode_ZEL_MIX,  "SYNC", &subdeco_sync_tof},
    {0, 0, 0}
};
struct decoder decolist_anke[]={
    {   1, decode_ignore, "ignored"},
    {   2, decode_ignore, "ignored"},
    {   3, decode_ignore, "ignored"},
    {   7, decode_ignore, "ignored"},
    {   8, decode_ignore, "ignored"},
    {  31, decode_ignore, "ignored"},
    {  47, decode_ignore, "ignored"},
    {  63, decode_ignore, "ignored"},
    {  79, decode_ignore, "ignored"},
    { 111, decode_ignore, "ignored"},
    { 127, decode_ignore, "ignored"},
    { 255, decode_ignore, "ignored"},
    {2788, decode_ignore, "ignored"},
    {2796, decode_ignore, "ignored"},
    {3871, decode_ZEL_MIX, "SYNCF1", &subdeco_sync_anke},
    {   0, 0, 0},
};
struct decoder *decolist=0;
/******************************************************************************/
static int
        ...
        u_int32_t evno, u_int32_t trigno, int size)
{
    ...
    if (deco->func)
        res=deco->func(po, &sev, queue, deco->aux);
    else
...
readargs(int argc, char* argv[])
{
    ...
    if (!strcmp(experiment, "tof")) {
        decolist=decolist_tof;
    } else if (!strcmp(experiment, "anke")) {
        decolist=decolist_anke;
    } else {
