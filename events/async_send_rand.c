 ...
 * async_send_rand.c
 * 
 * created 2011-08-02 PW
 * $ZEL: async_send_rand.c,v 1.1 2011/08/18 22:39:27 wuestner Exp $
 */
...
{
    printf("usage: %s [-h] [-q] [-4] -l maxlenbits destination\n", argv0);
    printf("  -h: this help\n");
...
readargs(int argc, char* argv[])
{
    ...
    while (!err && ((c=getopt(argc, argv, "hq46l:")) != -1)) {
        switch (c) {
        case 'h': printusage(argv[0]); return 1;
        case 'q': quiet=1; break;
        case '4': no_v6=1; break;
        case '6': no_v4=1; break;
        case 'l': maxlenbits=atoi(optarg); break;
        default: err=1;
        }
    }
    err=err||(no_v6&&no_v4);
    if (err || argc-optind!=1 || maxlenbits<=0) {
        printusage(argv[0]);
        return -1;
    }
...
main(int argc, char *argv[])
{
    ...
    maxlen=1<<(maxlenbits-1);
    maxlenmask=maxlen-1;
    if (!(buffer=malloc(((maxlen+3)/4)*4+4))) {
        perror("malloc");
        return 2;
    }
    for (i=0; i<maxlen; i++)
